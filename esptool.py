...
if __name__ == '__main__':
    ...
    parser_elf2ota = subparsers.add_parser(
            'elf2ota',
            help = 'Create an application image from ELF file')
    parser_elf2ota.add_argument('input', help = 'Input ELF file')
    parser_elf2ota.add_argument('--output', '-o', help = 'Output filename prefix', type = str)
    parser_elf2ota.add_argument('--flash_freq', '-ff', help = 'SPI Flash frequency',
            choices = ['40m', '26m', '20m', '80m'], default = '40m')
    parser_elf2ota.add_argument('--flash_mode', '-fm', help = 'SPI Flash mode',
            choices = ['qio', 'qout', 'dio', 'dout'], default = 'qio')
    parser_elf2ota.add_argument('--flash_size', '-fs', help = 'SPI Flash size in Mbit',
            choices = ['4m', '2m', '8m', '16m', '32m', '16m-c1', '32m-c1', '32m-c2'], default = '4m')
    ...
    esp = None
    if args.operation not in ('image_info','make_image','elf2image','elf2ota'):
    ...
    if args.operation == 'load_ram':
        ...
        image = ESPFirmwareImage()
        image.entrypoint = e.get_symbol_addr("call_user_start")
        for section, start in ((".text", "_text_start"), (".data", "_data_start"), (".rodata", "_rodata_start"), (".irom0.text", "_irom0_text_start")):
            data = e.load_section(section)
            image.add_segment(e.get_symbol_addr(start), data)
